...
async function generateListing(form,button,result,{auto=false}={}){
  const file=form.querySelector('input[type="file"]')?.files?.[0];
  if(!file){
    if(!auto){result.hidden=false;result.innerHTML='<b>Add a product photo first.</b>'}
    return;
  }
  ...
  result.hidden=false;
  result.innerHTML='<span>✦ AI is identifying the product and creating its listing…</span>';
  try{
    ...
    const r=await fetch('/api/catalog',{method:'POST',body:fd,signal:controller.signal});
    clearTimeout(timer);
    ...
    setValue(form.elements.craft,d.craft||d.buyer_category);
    ...
    const title=d.title||'Product listing';
    result.innerHTML=`<div class="ac-ai-ready">✓ AI listing ready</div><div class="ac-ai-generated"><b>${title}</b><span>${d.craft||d.buyer_category||'Handmade'}</span></div><div class="ac-ai-price"><span>Suggested selling price</span><strong>${rec?money(rec):'Add price manually'}</strong>${low&&high?`<small>Estimated range ${money(low)} – ${money(high)}</small>`:''}</div><p>${d.description||'Listing details generated from the uploaded product photo.'}</p><div class="ac-ai-meta"><span>Catalog confidence: ${Number(d.confidence||0)}%</span><span>Pricing confidence: ${Number(d.pricing_confidence||0)}%</span><span>Market match: ${Number(d.market_match_percentage||0)}%</span></div>${Array.isArray(d.tags)&&d.tags.length?`<small>Tags: ${d.tags.join(' · ')}</small>`:''}<small class="ac-ai-note">Review the AI suggestions before publishing.</small>`;
    form.dataset.aiCatalog=JSON.stringify(d);
  }catch(err){
    result.innerHTML=`<b>${err?.name==='AbortError'?'AI took too long. Tap Regenerate with AI.':String(err.message||'AI cataloging failed. Please try again.')}</b>`;
  }finally{
...
function enhance(){
  ...
  box.innerHTML=`<div><strong>✦ AI listing assistant</strong><small>Upload a product photo. AI will automatically suggest the product name, craft/category, materials, description, tags and selling price.</small></div><button type="button" class="btn secondary ac-ai-generate">Generate with AI</button><div class="ac-ai-result" hidden></div>`;
  const upload=form.querySelector('.artwork-upload')||form.querySelector('input[type="file"]')?.parentElement;
  ...
  button.addEventListener('click',()=>generateListing(form,button,result));

  const fileInput=form.querySelector('input[type="file"][accept*="image"]');
  fileInput?.addEventListener('change',()=>{
    if(!fileInput.files?.[0])return;
    result.hidden=false;
    result.innerHTML='<span>Photo added. Starting AI listing…</span>';
    setTimeout(()=>generateListing(form,button,result,{auto:true}),120);
  });
  return true;
}

const style=document.createElement('style');
style.textContent=`.ac-ai-catalog-box{margin:14px 0 18px;padding:14px;border:1px solid rgba(164,71,46,.22);border-radius:16px;background:rgba(164,71,46,.055);display:grid;gap:10px}.ac-ai-catalog-box>div:first-child{display:flex;flex-direction:column;gap:4px}.ac-ai-catalog-box small{color:#735a4c}.ac-ai-result{padding:12px;border-radius:12px;background:#fffaf2;display:grid;gap:8px}.ac-ai-ready{font-weight:900;color:#245b48}.ac-ai-generated{display:flex;flex-direction:column;gap:2px}.ac-ai-generated>b{font-size:16px}.ac-ai-price{display:flex;flex-direction:column;gap:2px}.ac-ai-price strong{font:700 24px Georgia,serif;color:var(--rust,#a4472e)}.ac-ai-meta{display:flex;gap:8px;flex-wrap:wrap}.ac-ai-meta span{font-size:11px;font-weight:700;padding:5px 8px;border-radius:999px;background:rgba(164,71,46,.08)}.ac-ai-note{font-style:italic}`;
