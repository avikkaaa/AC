...
function chooseItems(){
 ...
 const budgetOk=x=>{const p=fixedPrice(x.price);return p>=min&&p<=max};
 let exact=all.filter(x=>`${x.name} ${x.type} ${x.category}`.toLowerCase().includes(q));
 if(groups[q])exact=[...exact,...all.filter(x=>groups[q].some(g=>String(x.category).toLowerCase().includes(g.toLowerCase())))];
 if(!exact.length){const words=q.split(/\s+/).filter(x=>x.length>2);exact=all.filter(x=>words.some(w=>`${x.name} ${x.type} ${x.category}`.toLowerCase().includes(w)))}
 const combined=unique([...unique(exact).filter(budgetOk),...all.filter(budgetOk),...exact,...all]);
 combined.sort((a,b)=>(b.sellerEntered?1:0)-(a.sellerEntered?1:0));
 return{pref:pref||'handmade craft',items:combined.slice(0,18)};
}
...
function visual(item){
 if(item.sellerEntered&&item.image)return `<img src="${esc(item.image)}" alt="${esc(item.name)}" loading="lazy" decoding="async">`;
 return `<div class="ac-img-loading" aria-label="${esc(item.name)}"></div>`;
}
function checkout(item){
 document.querySelector('#ac-data-checkout')?.remove();const b=getBuyer(),price=money(fixedPrice(item.price)),m=document.createElement('div');
 m.id='ac-data-checkout';m.className='modal-backdrop';
 m.innerHTML=`<div class="login-modal"><button class="modal-close">×</button><span class="eyebrow">CHECKOUT</span><h2>${esc(item.name)}</h2><p><b>${price}</b> · fixed demo price</p><p><b>Deliver to</b><br>${esc(b.name||'Buyer')}<br>${esc(b.phone||'')}<br>${esc(b.address||'')}</p><label>Choose payment method<select id="ac-data-pay"><option>UPI</option><option>Card</option><option>Cash on Delivery</option></select></label><button class="modal-submit" id="ac-data-place">Place demo order →</button></div>`;
 document.body.appendChild(m);m.querySelector('.modal-close').onclick=()=>m.remove();m.onmousedown=e=>{if(e.target===m)m.remove()};
 m.querySelector('#ac-data-place').onclick=()=>{const order={id:Date.now(),title:item.name,craft:item.type,price,payment:m.querySelector('#ac-data-pay').value,buyer:b,status:'New'};let list=[];try{list=JSON.parse(localStorage.getItem('ac-demo-orders')||'[]')}catch{}list.push(order);localStorage.setItem('ac-demo-orders',JSON.stringify(list));m.querySelector('.login-modal').innerHTML=`<span class="eyebrow">ORDER CONFIRMED</span><h2>Order placed successfully!</h2><p>${esc(item.name)} · <b>${price}</b></p><p>This is a hackathon demo checkout. No real payment was charged.</p><button class="btn primary" id="ac-data-done">Continue shopping</button>`;m.querySelector('#ac-data-done').onclick=()=>m.remove()};
}
function render(){
 if(localStorage.getItem('ac-user-role')!=='buyer')return;const grid=document.querySelector('.buyer-products');if(!grid)return;
 const{items,pref}=chooseItems();if(!items.length)return;const sig=pref+'|'+items.map(x=>x.id).join(',');if(grid.dataset.catalogSig===sig)return;grid.dataset.catalogSig=sig;
 grid.innerHTML=items.map((x,i)=>`<article class="ac-ai-data-card" data-product-id="${esc(x.id)}"><div class="ac-ai-data-art">${visual(x)}</div><div class="buyer-product-info"><small>${esc(x.type)} · ${esc(x.location||'India')}</small><b>${esc(x.name)}</b><strong>${money(fixedPrice(x.price))}</strong><small>${Math.max(72,98-i*2)}% match</small><p>${esc(reason(x,pref))}</p><div class="ac-fixed-tag">${x.sellerEntered?`New seller listing · ${esc(x.artisan)}`:'Fixed price · uploaded catalog'}</div><button class="btn primary" data-buy="${esc(x.id)}">Buy now →</button></div></article>`).join('');
 window.AC_applyProductImages?.(grid);
 grid.querySelectorAll('[data-buy]').forEach(b=>b.onclick=()=>checkout(items.find(x=>x.id===b.dataset.buy)));
}
const style=document.createElement('style');style.textContent=`.ac-ai-data-card{overflow:hidden}.ac-ai-data-art{height:230px;position:relative;background:#ead3bd;overflow:hidden}.ac-ai-data-art>img{width:100%;height:100%;object-fit:cover;display:block}.ac-img-loading{width:100%;height:100%;background:#ead3bd}.ac-fixed-tag{display:inline-flex;align-self:flex-start;padding:5px 8px;border-radius:999px;background:rgba(188,82,43,.09);color:var(--rust);font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.04em}.ac-ai-data-card .buyer-product-info{height:auto}.ac-ai-data-card .buyer-product-info p{min-height:44px}`;document.head.appendChild(style);
window.addEventListener('storage',render);window.addEventListener('load',render,{once:true});document.addEventListener('click',()=>{setTimeout(render,80);setTimeout(render,700)},true);setTimeout(render,300);
