...
function chooseItems(){
  ...
  const preferred=unique(exact).filter(budgetOk);
  const sameBudget=all.filter(budgetOk);
  const combined=unique([...preferred,...sameBudget,...exact,...all]);
  combined.sort((a,b)=>(b.sellerEntered?1:0)-(a.sellerEntered?1:0));
  return{buyer:b,pref:pref||'handmade craft',items:combined.slice(0,18)}
}
...
function render(){if(localStorage.getItem('ac-user-role')!=='buyer')return;const grid=document.querySelector('.buyer-products');if(!grid)return;const{items,pref}=chooseItems();if(!items.length)return;const sig=pref+'|'+items.map(x=>x.id).join(',');if(grid.dataset.catalogSig===sig)return;grid.dataset.catalogSig=sig;grid.innerHTML=items.map((x,i)=>{const p=fixedPrice(x.price),match=Math.max(72,98-i*2);return `<article class="ac-ai-data-card"><div class="ac-ai-data-art ac-cat-${i%5}">${visual(x)}</div><div class="buyer-product-info"><small>${esc(x.type)} · ${esc(x.location||'India')}</small><b>${esc(x.name)}</b><strong>${money(p)}</strong><small>${match}% match</small><p>${esc(reason(x,pref))}</p>${x.sellerEntered?`<div class="ac-fixed-tag">New seller listing · ${esc(x.artisan)}</div>`:`<div class="ac-fixed-tag">Fixed price · uploaded catalog</div>`}<button class="btn primary" data-buy="${esc(x.id)}">Buy now →</button></div></article>`}).join('');grid.querySelectorAll('[data-buy]').forEach(b=>b.onclick=()=>checkout(items.find(x=>x.id===b.dataset.buy)))}
