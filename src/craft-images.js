...
function addRuntimeStyles(){if(document.getElementById('craft-image-runtime-styles'))return;const style=document.createElement('style');style.id='craft-image-runtime-styles';style.textContent='.craft-img,.portrait{position:relative;overflow:hidden}.craft-img img,.portrait img{width:100%;height:100%;display:block;object-fit:cover}.craft-img:after,.portrait:after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(36,22,18,0),rgba(36,22,18,.14));pointer-events:none}.portrait img{filter:saturate(.9) sepia(.05)}.artist-list article .portrait{width:130px;min-width:130px;height:150px;border-radius:0}.artist-list article .portrait img{width:100%;height:100%;object-fit:cover}.artist-list article{overflow:hidden}.artist-list article>div:last-child{padding:18px 20px}@media(max-width:1000px){.craft-grid{grid-template-columns:repeat(3,1fr)}}@media(max-width:650px){.craft-grid{grid-template-columns:repeat(2,1fr)}.craft-img{height:165px}.artist-list article .portrait{width:105px;min-width:105px;height:130px}}';document.head.appendChild(style)}
function addCraftImages(){addRuntimeStyles();document.querySelectorAll('.craft-grid .craft').forEach((card,i)=>{const image=craftImages[i],box=card.querySelector('.craft-img');if(!image||!box||box.dataset.imageAdded==='true')return;box.dataset.imageAdded='true';box.innerHTML='';const img=document.createElement('img');img.src=image[1];img.alt=image[2];img.loading='lazy';img.decoding='async';img.onerror=()=>{box.dataset.imageAdded='fallback';box.innerHTML='<span>✦</span>'};box.appendChild(img)})}
function addStoryImages(){document.querySelectorAll('.stories article').forEach((card,i)=>{const image=storyImages[i],box=card.querySelector('.portrait');if(!image||!box||box.dataset.imageAdded==='true')return;box.dataset.imageAdded='true';box.innerHTML='';const img=document.createElement('img');img.src=image[2];img.alt=image[3];img.loading='lazy';img.decoding='async';img.onerror=()=>{box.dataset.imageAdded='fallback';box.textContent=['✺','◌','✦'][i]||'✦'};box.appendChild(img);const title=card.querySelector('b'),meta=card.querySelector('small');if(title)title.textContent=image[0];if(meta)meta.textContent=image[1]})}
function addMakerImages(){document.querySelectorAll('.artist-list article').forEach((card,i)=>{const image=makerImages[i],box=card.querySelector('.portrait');if(!image||!box||box.dataset.makerImageAdded==='true')return;box.dataset.makerImageAdded='true';box.innerHTML='';const img=document.createElement('img');img.src=image[1];img.alt=image[2];img.loading='lazy';img.decoding='async';img.onerror=()=>{box.dataset.makerImageAdded='fallback';box.textContent='✦'};box.appendChild(img)})}
